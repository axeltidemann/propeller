...
with pd.HDFStore('onekyat.h5', mode='w') as store:
    for csv_file in args.csv:

        raw = pd.read_csv(csv_file, header=None, names=names, encoding='utf-8', nrows=nrows, index_col=0, error_bad_lines=False, engine='python')
        ...
        raw.images = raw.images.apply(lambda x: os.path.join(filepath, x[1:])) # leading / messes up join()
