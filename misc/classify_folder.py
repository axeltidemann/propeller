...
tf.app.flags.DEFINE_string('image_folder', '',
                           """Absolute path to folder of images.""")
...
def run_inference_on_folder(folder, top_k, target):
  ...
  with tf.Session() as sess:
    ...
    for image in glob.glob('{}/*jpg'.format(folder))[:20]:
      try:
        ...
        files[category].append(image)
        confidences[category].append(np.max(predictions))
      except Exception, e:
        print('File {} would not be read'.format(image))

    category_name = os.path.basename(folder.rstrip('/'))
...
def main(_):
  ...
  run_inference_on_folder(FLAGS.image_folder, FLAGS.num_top_classes, FLAGS.target)
